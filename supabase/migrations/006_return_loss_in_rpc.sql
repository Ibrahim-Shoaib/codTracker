-- ============================================================
-- Migration 006: surface total return loss in the dashboard RPC
-- ============================================================
-- The break-even card section needs Cost-per-Return = total return loss
-- divided by returned-order count. Total return loss is the same number
-- get_city_breakdown computes per-city; we now expose it as a single
-- aggregate on get_dashboard_stats so the front end has one source of
-- truth without summing across cities (which would also miss any cities
-- past the city-breakdown LIMIT 50 cap).
--
-- A "return loss" on an individual returned order is:
--   transaction_fee + transaction_tax  (forward shipping wasted)
-- + reversal_fee  + reversal_tax       (reverse shipping paid)
-- + cogs_total × (1 − sellable_pct/100)  (unsellable inventory)
--
-- This matches get_city_breakdown.return_loss line-for-line, so the
-- dashboard's "Cost per Return" card and the city panel's loss numbers
-- stay consistent.
--
-- Profit math is unchanged — return_loss is a *display* aggregate, not
-- a new cost subtraction. v_gross_profit and v_net_profit are byte-
...
RETURNS TABLE (
  ...
  net_profit     numeric,
  return_loss    numeric,   -- NEW: total PKR cost of returned shipments in window
  roas           numeric,
  poas           numeric,
  cac            numeric,
  aov            numeric,
...
BEGIN
  ...
  SELECT
    ...
    -- return_loss: real PKR cost on each returned shipment.
    -- Same formula as get_city_breakdown so the two stay numerically identical.
    COALESCE(SUM(
      CASE WHEN o.is_returned
        THEN (o.transaction_fee + o.transaction_tax + o.reversal_fee + o.reversal_tax)
